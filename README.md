# ᛉᛟᛣϟϟ Valhalla Exhausts ᛉᛟᛣϟϟ

Landing page corporativa para **Valhalla Exhausts**, una marca centrada en escapes artesanales a medida, reparación de sistemas de escape y soluciones personalizadas para automoción.

La web presenta un diseño oscuro, visual y enfocado a conversión, mostrando servicios, proceso de trabajo, beneficios, preguntas frecuentes y vías de contacto.

---

## Vista previa 𓄿

- Hero principal con mensaje de marca
- Sección de presentación de servicios
- Explicación del proceso de trabajo
- Beneficios y propuesta de valor
- FAQ
- Formas de contacto directas

---

## Características ﹏𓊝﹏

- Diseño responsive orientado a móvil
- Estética premium y oscura
- Estructura de landing page pensada para captar clientes
- Secciones informativas claras y escaneables
- Integración con canales de contacto
- Enfoque visual para transmitir calidad y especialización

---



